...
dp_range<-function(x,sd,epsilon,delta=0,bound.mean,range.prob){
  ...
  if(length(unique(discretized.x))==1){
    ...
    hist.df=dp_perturbed_hist(hist.df=hist.df.orig,epsilon=epsilon,delta=delta,sensitivity.multiplier=2,quietly=T)
    #san.props=hist.df[[2]]
    #hist.df=hist.df[[1]]
    ...
    if(length(hist.df[which.max(hist.df$san.prop),1])==0){
      ...
      if(length(hist.df[which.max(hist.df$san.prop),1])==0){
        new.sd=dp_estimate_sd(x,epsilon/2,delta/2,c(2^(-15),2^(15)))
        # warning(paste(
        #   "orginial hist.df is",paste0(hist.df.orig[hist.df.orig$san.prop==max(hist.df.orig$san.prop,na.rm=T),],collapse=", "),
...
dp_estimate_sd<-function(x,epsilon,delta=0,bounds.sd){
  ...
  hist.df=dp_perturbed_hist(hist.df=hist.df,epsilon=epsilon,delta=delta,sensitivity.multiplier=2,quietly=T)
